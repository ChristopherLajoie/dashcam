...
class ParameterHandler:
    ...
    def _set_onvif_param(self, param: str, value, extra: tuple = None):
        try:
            ...
            imaging_settings = self.imaging_service.GetImagingSettings(
                self.source_token
            )
            ...
            if self.ONVIF_PARAMS[param] == "imaging":
                ...
                self.media_service.SetVideoEncoderConfiguration(
                    {"Configuration": config_dict, "ForcePersistence": True}
                )
    ...
    def _set_http_param(
        self, param: str, value: str, extra: str = None, force: bool = False
    ):
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(
                    f"Failed to update {param} via HTTP. Status: {response.status_code}"
                )
                return False
        except Exception as e:
